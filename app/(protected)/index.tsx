...
export default function Home() {
  ...
  const canAccessPremiumCourse = (course: UserCourse): boolean => {
    ...
    const userWeekPoints =
      course.userWeekPoints ?? course.course.userWeekPoints ?? weeklyPoints;
    const weekPointsRequired =
      course.weekPointsRequired ??
      course.course.weekPointsRequired ??
      course.course.premiumPoints;
  ...
  const handleCoursePress = (course: UserCourse) => {
    ...
    // Navegação será feita via Link no Pressable
  };
              ...
              <View className="mb-6">
                ...
                <View className="space-y-3">
                  {userCourses.map((course) => {
                    const isPremium =
                      course.course.premiumPoints &&
                      course.course.premiumPoints > 0;
                    const canAccess = canAccessPremiumCourse(course);

                    const canNavigate =
                      isSignedIn && canAccessPremiumCourse(course);

                    return canNavigate ? (
                      <Link
                        key={course.course._id}
                        href={`/curso/${course.course._id}` as any}
                        asChild
                      >
                              ...
                              <View className="flex-row items-center justify-between mb-2">
                                ...
                                {isPremium && (
                                  <View className="flex-row items-center ml-2">
                                    ...
                                    <Link href="/curso-premium-info" asChild>
                                      <Pressable
                                        onPress={(e) => {
                                          e.stopPropagation();
                                        }}
                                ...
                                <View className="mt-3 bg-yellow-50 border border-yellow-200 rounded-lg p-2">
                                  <Text className="text-yellow-800 text-xs text-center">
                                    ...
                                    {course.weekPointsRequired ??
                                      course.course.weekPointsRequired ??
                                      course.course.premiumPoints}{" "}
                                    pontos semanais (você tem{" "}
                                    {course.userWeekPoints ??
                                      course.course.userWeekPoints ??
                                      weeklyPoints}
                                    )
                                  </Text>
                                </View>
                          ...
                          >
                            <View className="bg-card rounded-lg border border-border overflow-hidden">
                              {course.course.banner?.asset?.url && (
                                <Image
                                  source={{
                                    uri: course.course.banner.asset.url,
                                  }}
                                  className="w-full h-40"
                                  resizeMode="cover"
                                />
                                ...
                                {course.progress && (
                                  <View className="mt-2">
                                    <View className="flex-row items-center justify-between mb-1">
                                      ...
                                      <Text className="text-text-secondary text-xs">
                                        {course.progress.percentage?.toFixed(
                                          0
                                        ) ?? 0}
                                        %
                                      </Text>
                                    ...
                                    <Text className="text-text-secondary text-xs mt-1">
                                      ...
                                      de {course.progress.totalLectures ?? 0}{" "}
                                      aulas
                                    </Text>
                                  </View>
                                )}
